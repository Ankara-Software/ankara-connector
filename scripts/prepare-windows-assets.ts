...
/** Status page uses the wide logo (better contrast on dark UI). */
const statusLogo = PNG.sync.read(wide);
writeFileSync(join(ASSETS, 'status-logo.png'), PNG.sync.write(statusLogo));
...
export const STATUS_LOGO_PNG = Buffer.from('${PNG.sync.write(statusLogo).toString('base64')}', 'base64');
`);
